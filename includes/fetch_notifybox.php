...
switch ($what) {
	...
	$notify_sql = "SELECT notifications.*,myclass.name FROM notifications,wpost,myclass WHERE for_id =:myId AND notifications.for_id = wpost.author_id AND myclass.id = wpost.class_id AND myclass.id = :class_id ORDER BY time DESC LIMIT :load,10";

	$notify = $conn->prepare($notify_sql);
	$notify->bindParam(':myId',$myId,PDO::PARAM_INT);
	$notify->bindParam(':class_id',$class_id,PDO::PARAM_INT);
	$notify->bindValue(':load', (int)trim($load), PDO::PARAM_INT);
	$notify->execute();
	$notifyCount = $notify->rowCount();
	if ($notifyCount > 0) {
		while ($n_row = $notify->fetch(PDO::FETCH_ASSOC)) {
			$class_name = " at ".$n_row['name'];
			...
			switch ($notifyType) {
				...
				<p style='font-size:13px;'><b>$fullname$class_name</b> ".lang('likeNotify_str')." <span style='color: #999;'>$getPCon</span>
				...
				<p style='font-size:13px;'><b>$fullname$class_name</b> ".lang('commmentNotify_str').".
				<span style='font-size: small;'></span><br>
				...
				<p style='font-size:13px;'><b>$fullname$class_name</b> ".lang('shareNotify_str')." <span style='color: #999;'>$getPCon</span>
				<span style='font-size: small;'></span><br>
...
// =============================================================
	case 'class':
	$getClass = $conn->prepare("SELECT * FROM myclass, mystudy WHERE myclass.id = mystudy.class_id AND mystudy.id=:myId ");
	$getClass->bindParam(':myId',$myId,PDO::PARAM_INT);
	$getClass->execute();
	while ($class_row = $getClass->fetch(PDO::FETCH_ASSOC)) {
		$pId = $class_row['class_id'];
		$pName = $class_row['name'];
		$pMaLop = $class_row['malop'];
		echo "
		<a href='".$path."u/".$pUsername."' onclick=\"getAClass('$pId')\">
		<div id='sqresultClassItem'>
		<div style='display: inline-flex;width: 100%;'>
		<p style='font-size:15px;'><b>$pName ($pMaLop)</b>
		<span style='font-size: small;'></span><br>
		</p>
		</div>
		</div>
...
function getAClass(pId){
	$.post("includes/c_action.php",{id_Class: pId}, function(data){});
