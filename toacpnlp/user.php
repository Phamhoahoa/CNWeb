...
$uClass = $conn->prepare("SELECT distinct myclass.* from mystudy,myclass,signup where myclass.id = mystudy.class_id AND mystudy.id = signup.id AND signup.Username = :ed");
$uClass->bindParam(':ed',$ed,PDO::PARAM_STR);
$uClass->execute();
$uClass_conut = $uClass->rowCount();

$uAddClass = $conn->prepare("SELECT distinct * from myclass where id not in (SELECT distinct myclass.id from mystudy,myclass,signup where mystudy.id = signup.id AND signup.Username = :ed AND myclass.id = mystudy.class_id)");
$uAddClass->bindParam(':ed',$ed,PDO::PARAM_STR);
$uAddClass->execute();
$uAddClass_count = $uAddClass->rowCount();
// update user info
...
if (isset($_POST['rAccBtn'])) {
    if (empty($_POST['rAccField'])) {
        ...
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM mystudy WHERE id= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        if ($remeveAccount) {
...
<div class="dashboard_box" style="text-align: <? echo lang('textAlign'); ?>">
    ...
    <form action="" method="post">
        <table>
            ...
            <tr>
                <td>Classes:</td>
                <td><select class="dashboardField">
                    <option></option>
                    <?php
                    if ($uClass_conut > 0) {
                        while ($uClass_row = $uClass->fetch(PDO::FETCH_ASSOC)) {
                            $uClass_name = $uClass_row['name'];
                            $uClass_malop = $uClass_row['malop'];
                            echo "<option>$uClass_name - $uClass_malop</option>";
                        }
                    }
                    ?>
                </select></td>
