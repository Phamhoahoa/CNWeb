...
// ============= [ Add Class   ] ==============
//get var's
$db_classname = trim(filter_var(htmlspecialchars($_POST['classname']),FILTER_SANITIZE_STRING));
$db_malop = trim(filter_var(htmlspecialchars($_POST['malop']),FILTER_SANITIZE_STRING));
if(isset($_POST['submit_addClass'])){
    $addClass = $conn->prepare("SELECT malop from myclass where malop = :malop");
    $addClass->bindParam(':malop',$db_malop,PDO::PARAM_STR);
    $addClass->execute();
    while($row_addClass = $addClass->fetch(PDO::FETCH_ASSOC)){
        $row_addClass_malop = $row_addClass['malop'];
    }
    if(empty($row_addClass_malop)){
        $addClass = $conn->prepare("INSERT INTO myclass (name,malop) values (:name,:malop)");
        $addClass->bindParam(':name',$db_classname,PDO::PARAM_STR);
        $addClass->bindParam(':malop',$db_malop,PDO::PARAM_STR);
        $addClass->execute();
    }else{
...
<head>
    ...
    <meta charset="UTF-8">
    <meta name="description" content="HustWeb is a social network platform helps you meet new friends and stay connected with your family and with who you are interested anytime anywhere.">
...
<div class="dashboard_box" style="text-align: <? echo lang('textAlign'); ?>">
    ...
    <!--///////////////Classes ////////////////////////-->
    <?php case 'Classes': ?>
    <p class="dashboard_path"><a href="?adb=General"><? echo lang('dashboard'); ?></a> / Classes</p>
    <br/>
    <form action="" method="post">
        <table class="dashboard_UsersTable">
            ...
            outline: none;
            border-radius: 2px;
            margin: 5px;" dir="auto" type="text" name="classname" class="dashboardClassField" placeholder='Classname'></td>
            <td><input style = "width: 150px;
            height: 28px;
            background: rgba(255, 255, 255, 0.61);
            border: 1px solid #eeeeee;
            ...
            outline: none;
            border-radius: 2px;
            margin: 5px;" dir="auto" type="text" name="malop" class="dashboardClassField" placeholder='Ma Lop'></td>
            <td align="center" style="width: 150px;"><input style="width: 70px;
                ...
                border: none;" type="submit" name="submit_addClass"></td>
        </table>
    </form>
    <?php
    $fetchClasses_sql = "SELECT * FROM myclass";
    $fetchClasses = $conn->prepare($fetchClasses_sql);
    $fetchClasses->execute();
    while ($rows = $fetchClasses->fetch(PDO::FETCH_ASSOC)) {
    ?>
    <table id = "class_id_<?php echo $rows['id']; ?>" class="dashboard_UsersTable">
        <tr>
        <td style="width: 40px;">
            <div><img src="<?php echo $dircheckPath; ?>imgs/main_icons/class_main_icon.png"></div>
        </td>
        <td>
            <a href = "userInClass?ed=<?php echo $rows['id']; ?>"><p><?php echo $rows['name']; ?> ___ <?php echo $rows['malop']; ?></p></a>
        </td>
        <td align="center" style="width: 150px;"><a onclick = "delete_class(<?php echo $rows['id']; ?>)" href='#' class="dashboard_EditDelete" >Delete</a></td>
        </tr>
    </table>
    <?php };?>
    <? echo $add_result ?>
...
<script type="text/javascript">
    function delete_class(id){
        $.ajax({
            type: "POST",
            url: "<?php echo $dircheckPath; ?>includes/deleteclass.php",
            data: {'id':id},
            success: function(html){
                classId = "#class_id_"+id.toString();
                console.log(classId);
                $(classId).remove();
            }
        });
